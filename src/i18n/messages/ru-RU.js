/* eslint-disable import/no-anonymous-default-export */
import {LOCALES} from '../locales';

export default {
	[LOCALES.RUSSIAN]: {
		'employees-title': 'Сотрудники',
		'birthdays-title': 'День рождения сотрудников',
		'no-employees': 'Нет сотрудников',
		'list-empty': 'Список сотрудников пуст',

		'active': 'активный',
		'not-active': 'не активен',

		'home-menu': 'Главная',
		'employees-menu': 'Сотрудники',
		'charts-menu': 'Диаграммы',
		'profile-menu': 'Профиль',
		'auth-menu': 'Auth',
		'logout-menu': 'Logout',

		'footer-text': 'Все права защищены',

		'test-task': 'Тестовое задание',
		'company-name': 'Yalantis',
		'react': 'React',
		'school': 'School',

		'january': 'Январь',
		'february': 'Февраль',
		'march': 'Март',
		'april': 'Апрель',
		'may': 'Май',
		'june': 'Июнь',
		'july': 'Июль',
		'august': 'Август',
		'september': 'Сентябрь',
		'october': 'Октябрь',
		'november': 'Ноябрь',
		'december': 'Декабрь',
		'year': ' год',

		'title-charts': "Дни рождения сотрудников по месяцам",
	}
}